...
export default function Support() {
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>, isReply = false) => {
    ...
    const toastId = toast.loading(isBn ? "ক্লাউডে ফাইল আপলোড হচ্ছে..." : "Uploading attachment to cloud CDN...");
    try {
      const cdnUrl = await uploadImage(file);
      if (isReply) {
        setReplyAttachment(cdnUrl);
      ...
      toast.success(isBn ? "ফাইল সফলভাবে ক্লাউডে আপলোড হয়েছে!" : "Attachment uploaded to cloud CDN!", { id: toastId });
    } catch (err: any) {
      console.error("[Support attachment upload error]:", err);
      toast.error(isBn ? "ফাইল আপলোড করতে সমস্যা হয়েছে।" : "Failed to upload attachment.", { id: toastId });
    } finally {
...
function TicketConversationThread({
  ...
  const handleFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      const cdnUrl = await uploadImage(file);
      setReplyFileBase64(cdnUrl);
      setReplyFileName(file.name);
      toast.success(isBn ? "ফাইল ক্লাউডে আপলোড হয়েছে!" : "File uploaded to cloud CDN!", { id: toastId });
    } catch (err: any) {
      console.error("[Reply attachment upload error]:", err);
